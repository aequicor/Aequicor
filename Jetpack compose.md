...
```
 далее удаляем папку build модуля и вызываем град таску `./gradlew :модуль где добавлены:assable` или просто собираем проект. Открываем файл с суффиксом `common-composables.txt` и **находим методы принимающие типы как параметры и смотрим, что они стабильные**